...
modelPath = currFile.replace('/synthesis/FACTEST/omega_FACTEST/nursery-3d.py', '')
sys.path.append(modelPath)
sys.path.append(modelPath+'/utils/')
modelPath = currFile.replace('/nursery-3d.py', '')
sys.path.append(modelPath)
...
if system == 'hovercraft':
    ...
    my_id = "myCar"
    my_code = None
    my_filename = currFile.replace('synthesis/FACTEST/omega_FACTEST/nursery-3d.py', 'models/dubins/hovercraftControl.py')
    ...
    my_id = "myCar"
    my_code = None
    my_filename = currFile.replace('synthesis/FACTEST/omega_FACTEST/nursery-3d.py', 'models/dubins/auvControl.py')
...
b_goal2 = np.array([6.5,-5.5,6.5,-5.5, -5.5,6.5])
b_goal3 = np.array([-5.5,6.5,6.5,-5.5, -5.5,6.5])
...
b_unsafe10 = np.array([11,11,11,11,11,-10])
b_unsafe11 = np.array([11,11,11,11,-10,11])

b_unsafe5 = np.array([3,-2,11,-7, 11,11])
b_unsafe7 = np.array([3,-2,-7,11, 11,11])

b_unsafe6 = np.array([-5,11,1,1, 11,11])

b_unsafe8 = np.array([-2,3,-7,11, 11,11])
b_unsafe9 = np.array([-2,3,11,-7, 11,11])

initial_set = pc.Polytope(A, b_init)
E1 = pc.Polytope(A, b_goal1) # goal set 1, baby
E2 = pc.Polytope(A, b_goal2) # goal set 2, charger
E3 = pc.Polytope(A, b_goal3) # goal set 3, adult


E4 = pc.Polytope(A, b_unsafe5) # unsafe set 1
E5 = pc.Polytope(A, b_unsafe6) # unsafe set 2
E6 = pc.Polytope(A, b_unsafe7) # unsafe set 3
E7 = pc.Polytope(A, b_unsafe8) # unsafe set 4
E8 = pc.Polytope(A, b_unsafe9) # unsafe set 5
...
E13 = pc.Polytope(A, b_unsafe10)
E14 = pc.Polytope(A, b_unsafe11)

env = {'b':E1, 'c':E2, 'a':E3, 'o1':E4, 'o2':E5, 'o3':E6, 'o4':E7, 'o5':E8}
...
str2 = 'G ((b & !(X b)) -> (X !b U (a | c)))'
str3 = 'G (a -> X (!a U b))'
str4 = 'G ((!b & X b & !(X (X b))) -> (!a U c))'
str5 = 'G (c -> (!a U b))'
str6 = 'G ((b & X b) -> F a)'

avoid_str = 'G (!o1 & !o2 & !o3 & !o4 & !o5)'

ltl_formula = str2 + ' & ' + str3 + ' & ' + str4 + ' & ' + str5 + ' & ' + str6 + ' & ' + avoid_str
...
import csv
with open('/Users/krismiller/Desktop/dissertation/synthesis/FACTEST/omega_FACTEST/results/nursery_'+system+'.csv', 'w', newline='') as csvfile:
